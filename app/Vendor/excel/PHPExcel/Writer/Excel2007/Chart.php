...
class PHPExcel_Writer_Excel2007_Chart extends
  ...
  private function _writePlotArea(PHPExcel_Chart_PlotArea $plotArea,
    ...
    foreach ($chartTypes as $chartType) {
      ...
      for ($i = 0; $i < $groupCount; ++$i) {
        ...
        $GroupClass = $plotGroup->getPlotType();
        if ($GroupClass == $chartType) {

          $plotStyle = $plotGroup->getPlotStyle();
          if ($GroupClass === PHPExcel_Chart_DataSeries::TYPE_RADARCHART) {
            $objWriter->startElement('c:radarStyle');
            $objWriter->writeAttribute('val', $plotStyle);
            $objWriter->endElement();
          } elseif ($GroupClass === PHPExcel_Chart_DataSeries::TYPE_SCATTERCHART) {
   ...
   * @param  PHPExcel_Chart_PlotArea $plotArea
   * @param  PHPExcel_Chart_Title $xAxisLabel
   * @param  string $GroupClass Chart type
   * @param  string $id1
   * @param  string $id2
   ...
   * @throws  PHPExcel_Writer_Exception
   */
  private function _writeCatAx($objWriter, PHPExcel_Chart_PlotArea $plotArea, $xAxisLabel, $GroupClass, $id1, $id2, $isMultiLevelSeries, $xAxis, $yAxis) {
   ...
   * @throws  PHPExcel_Writer_Exception
   */
  private function _writeValAx($objWriter, PHPExcel_Chart_PlotArea $plotArea, $yAxisLabel, $GroupClass, $id1, $id2, $isMultiLevelSeries, $xAxis, $yAxis, $majorGridlines, $minorGridlines) {
    ...
    if (!is_null($yAxisLabel)) {
      ...
      if ($GroupClass !== PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) {
   ...
   * @param  string $GroupClass Type of plot for dataseries
   * @param  PHPExcel_Shared_XMLWriter $objWriter XML Writer
  ...
  private function _writePlotGroup($plotGroup,
    ...
    if (($GroupClass == PHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
        ($GroupClass == PHPExcel_Chart_DataSeries::TYPE_BARCHART_3D)
    ) {
    ...
    if (($GroupClass !== PHPExcel_Chart_DataSeries::TYPE_RADARCHART) &&
        ($GroupClass !== PHPExcel_Chart_DataSeries::TYPE_STOCKCHART)
    ) {

      if ($GroupClass !== PHPExcel_Chart_DataSeries::TYPE_LINECHART) {
        if (($GroupClass == PHPExcel_Chart_DataSeries::TYPE_PIECHART) ||
            ($GroupClass == PHPExcel_Chart_DataSeries::TYPE_PIECHART_3D) ||
            ($GroupClass == PHPExcel_Chart_DataSeries::TYPE_DONUTCHART) ||
            ($plotSeriesCount > 1)
        ) {
    ...
    foreach ($plotSeriesOrder as $plotSeriesIdx => $plotSeriesRef) {
      ...
      if (($GroupClass == PHPExcel_Chart_DataSeries::TYPE_LINECHART) ||
        ...
        if ($GroupClass == PHPExcel_Chart_DataSeries::TYPE_STOCKCHART) {
      ...
      if (($GroupClass === PHPExcel_Chart_DataSeries::TYPE_BARCHART) ||
          ($GroupClass === PHPExcel_Chart_DataSeries::TYPE_BARCHART_3D) ||
          ($GroupClass === PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART)
      ) {
      ...
      if ($plotSeriesCategory && ($plotSeriesCategory->getPointCount() > 0)) {
        ...
        if (($GroupClass === PHPExcel_Chart_DataSeries::TYPE_BUBBLECHART) ||
            ($GroupClass === PHPExcel_Chart_DataSeries::TYPE_SCATTERCHART)
        ) {
        ...
        $this->_writePlotSeriesValues($plotSeriesCategory, $objWriter, $GroupClass, 'str', $pSheet);
        $objWriter->endElement();
      }
      ...
      if ($plotSeriesValues) {
        ...
        $this->_writePlotSeriesValues($plotSeriesValues, $objWriter, $GroupClass, 'num', $pSheet);
        $objWriter->endElement();
      }
